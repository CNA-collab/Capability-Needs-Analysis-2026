...
export const parseCorporatePlanFile = (file: File): Promise<{ data: StructuredCorporatePlan }> => {
    return new Promise((resolve, reject) => {
        ...
        if (!supportedFormats.includes(fileFormat)) {
            reject(new Error(`Unsupported file format. Supported formats: ${supportedFormats.join(', ')}`));
            return;
        }
        ...
        reader.onload = (e) => {
            try {
                ...
                switch (fileFormat) {
                    ...
                    case '.docx':
                        import('mammoth').then((mammoth: unknown) => {
                            (mammoth as { extractRawText: (options: { path: string }) => Promise<{ value: string }> }).extractRawText({ path: file.name }).then((result) => {
                                try {
                                    const text = result.value;
                                    if (!text || text.trim().length === 0) {
                                        reject(new Error('The DOCX file appears to be empty or contains no extractable text.'));
                                        return;
                                    }
                                    const corporatePlanData = parseCorporatePlanText(text);
                                    resolve({ data: corporatePlanData });
                                    ...
                                    reject(new Error('Failed to parse the DOCX content. The document may be corrupted or in an unsupported format.'));
                                }
                            }).catch((docxError: unknown) => {
                                console.error('DOCX extraction error:', docxError);
                                reject(new Error('Failed to extract text from DOCX. Please ensure the file is a valid Word document.'));
                            });
                        }).catch((importError: unknown) => {
                            console.error('DOCX library import error:', importError);
                            reject(new Error('DOCX processing library failed to load. Please refresh the page and try again.'));
                        });
                        break;
                    ...
                    case '.txt':
                        try {
                            ...
                            reject(new Error('Failed to parse the text file.'));
                        }
                        break;

                    case '.html':
                        import('htmlparser2').then((htmlparser: unknown) => {
                            const { parseDOM, DomHandler } = (htmlparser as any);
                            try {
                                const handler = new DomHandler((error, dom) => {
                                    ...
                                    const text = dom.reduce((acc, node) => {
                                        if (node.type === 'text') {
                                    ...
                                    if (!text || text.trim().length === 0) {
                                        reject(new Error('The HTML file appears to be empty or contains no extractable text.'));
                                        return;
                                    }
                                    const corporatePlanData = parseCorporatePlanText(text);
                                    resolve({ data: corporatePlanData });
                                ...
                                reject(new Error('Failed to parse the HTML content.'));
                            }
                        }).catch((importError: unknown) => {
                            console.error('HTML library import error:', importError);
                            reject(new Error('HTML processing library failed to load. Please refresh the page and try again.'));
                        });
                        break;

                    default:
                        reject(new Error(`Unsupported file format: ${fileFormat}`));
                }
            } catch (err) {
