...
interface HtmlParser2 {
  DomHandler: new (callback: (error: Error | null, dom: any[]) => void) => void;
  parseDOM: (html: string, handler: any) => void;
}

// HTML parser types
interface DomHandler {
  new (callback: (error: Error | null, dom: any[]) => void): void;
}

interface HtmlParser {
  parseDOM: (html: string, handler: DomHandler) => void;
}
...
class FileSizeError extends Error {
  constructor(size: number) {
    super(`File too large: ${size} bytes (Limit: ${FILE_SIZE_LIMIT} bytes)`);
...
class UnsupportedFormatError extends Error {
  constructor(format: string) {
    super(`Unsupported file format: ${format}. Supported formats: ${SUPPORTED_FORMATS.join(', ')}`);
...
// File parsers
const parseCnaFile = async (file: File, agencyType: AgencyType): Promise<{ data: OfficerRecord[], preview: unknown[], headers: string[] }> => {
  ...
  if (!SUPPORTED_FORMATS.includes(fileFormat as any)) {
    throw new UnsupportedFormatError(fileFormat);
  }
  ...
  return new Promise((resolve, reject) => {
    reader.onload = async (e) => {
      try {
        ...
        reject(new FileParseError('Failed to parse XLSX file', 'XLSX'));
      }
    };

    reader.onerror = () => {
      reject(new FileParseError('Failed to read file', 'READER'));
    };
...
const parseEstablishmentFile = async (file: File, agencyType: AgencyType): Promise<{ data: EstablishmentRecord[] }> => {
  if (file.size > FILE_SIZE_LIMIT) {
    throw new FileSizeError(file.size);
  }
...
const parsePastedData = async (pastedText: string, agencyType: AgencyType): Promise<{ data: OfficerRecord[], preview: unknown[], headers: string[] }> => {
  if (!pastedText.trim()) {
    throw new Error('Pasted data cannot be empty');
  }

  const rows = pastedText.split('\n').map(row => row.split('\t'));
  if (rows.length < 2) {
    throw new Error('Pasted data must contain at least a header row.');
  }
...
const parseCorporatePlanFile = async (file: File): Promise<{ data: StructuredCorporatePlan }> => {
  if (file.size > FILE_SIZE_LIMIT) {
    throw new FileSizeError(file.size);
  }
  ...
  return new Promise((resolve, reject) => {
    reader.onload = async (e) => {
      try {
        ...
        switch (fileFormat) {
          case '.pdf':
            import('pdf-parse').then((pdfParse: any) => {
              (pdfParse as { default: (data: Uint8Array) => Promise<{ text: string }> }).default(data).then((pdfData) => {
                try {
                  const text = pdfData.text;
                  if (!text || text.trim().length === 0) {
                    reject(new FileParseError('The PDF file appears to be empty or contains no extractable text. Please ensure the file is not password-protected or corrupted.', 'PDF'));
                    return;
                  }
                  ...
                  reject(new FileParseError('Failed to parse the PDF content. The document may be corrupted or in an unsupported format.', 'PDF'));
                }
              }).catch((pdfError: unknown) => {
                console.error('PDF extraction error:', pdfError);
                reject(new FileParseError('Failed to extract text from PDF. Please ensure the file is a valid PDF document.', 'PDF'));
              });
            }).catch((importError: unknown) => {
              console.error('PDF library import error:', importError);
              reject(new FileParseError('PDF processing library failed to load. Please refresh the page and try again.', 'PDF'));
            });
            break;

          case '.docx':
            import('mammoth').then((mammoth: any) => {
              (mammoth as { extractRawText: (options: { path: string }) => Promise<{ value: string }> }).extractRawText({ path: file.name }).then((result) => {
                try {
                  const text = result.value;
                  if (!text || text.trim().length === 0) {
                    reject(new FileParseError('The DOCX file appears to be empty or contains no extractable text.', 'DOCX'));
                    return;
                  }
                  const corporatePlanData = parseCorporatePlanText(text);
                  resolve({ data: corporatePlanData });
                  ...
                  reject(new FileParseError('Failed to parse the DOCX content. The document may be corrupted or in an unsupported format.', 'DOCX'));
                }
              }).catch((docxError: unknown) => {
                console.error('DOCX extraction error:', docxError);
                reject(new FileParseError('Failed to extract text from DOCX. Please ensure the file is a valid Word document.', 'DOCX'));
              });
            }).catch((importError: unknown) => {
              console.error('DOCX library import error:', importError);
              reject(new FileParseError('DOCX processing library failed to load. Please refresh the page and try again.', 'DOCX'));
            });
            break;
          ...
          case '.txt':
            try {
              ...
              if (!text || text.trim().length === 0) {
                reject(new FileParseError('The text file appears to be empty.', 'TXT'));
                return;
              }
              const corporatePlanData = parseCorporatePlanText(text);
              resolve({ data: corporatePlanData });
            } catch (parseError) {
              console.error('TXT parsing error:', parseError);
              reject(new FileParseError('Failed to parse the text file.', 'TXT'));
            }
            break;

          case '.html':
            import('htmlparser2').then((htmlparser2) => {
              try {
                // htmlparser2 v10+ exports: parseDOM as default, DomHandler as named export
                const { DomHandler, parseDOM } = htmlparser2 as { DomHandler: new (callback: (error: Error | null, dom: any[]) => void) => void; parseDOM: (html: string, handler: any) => void };
                const handler = new DomHandler((error: Error | null, dom: any[]) => {
                  if (error) {
                    reject(new FileParseError('Failed to parse HTML content.', 'HTML'));
                    return;
                  }
                  const text = dom.reduce((acc: string, node: any) => {
                    if (node.type === 'text') {
                  ...
                  if (!text || text.trim().length === 0) {
                    reject(new FileParseError('The HTML file appears to be empty or contains no extractable text.', 'HTML'));
                    return;
                  }
                  const corporatePlanData = parseCorporatePlanText(text);
                  resolve({ data: corporatePlanData });
                });
                parseDOM(new TextDecoder().decode(data), handler);
              } catch (parseError) {
                console.error('HTML parsing error:', parseError);
                reject(new FileParseError('Failed to parse the HTML content.', 'HTML'));
              }
            }).catch((importError: unknown) => {
          ...
          default:
            reject(new UnsupportedFormatError(fileFormat));
        }
      } catch (err) {
        console.error('File reading error:', err);
        reject(new FileParseError('Failed to read the uploaded file. Please try uploading again.', 'READER'));
      }
    };

    reader.onerror = () => {
      reject(new FileParseError('Failed to read the file. Please check your file and try again.', 'READER'));
    };
